...
exports.formatAPDU = (value:DPT2Value) => {
  if (value == null) return console.log('DPT2: cannot write null value');

  if (
    typeof value === 'object' &&
    'priority' in value && 'data' in value //@zhri -> changed 'hasOwnProperty' with 'in' to check property presence
  )
    return Buffer.from([(value.priority << 1) + (value.data & 0b00000001)]);

  console.log('DPT2: Must supply an value {priority:<bool>, data:<bool>}');
  // FIXME: should this return zero buffer when error? Or nothing?
  return Buffer.from([0]);
};

exports.fromBuffer = (buf:Buffer) => {
  if (buf.length !== 1) return console.log('Buffer should be 1 byte long');
...
exports.subtypes = {
  ...
  // 2.010 start control
  '010': { //@zhri fix the bit range '001' to '010'
    use: 'FB',
  ...
  // 2.011 state control
  '011': {   //@zhri fix the bit range '001' to '011'
    use: 'FB',
    name: 'DPT_Switch_Control',
    desc: 'switch',
    enc: { 0: 'No control', 1: 'No control', 2: 'Off', 3: 'On' },
  ...
  '012': {  //@zhri fix the bit range '001' to '012'
    use: 'FB',
    name: 'DPT_Switch_Control',
    desc: 'switch',
    enc: { 0: 'No control', 1: 'No control', 2: 'Off', 3: 'On' },
